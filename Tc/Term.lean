...
def frost     : UInt32 := 110   -- nord ice blue (#87afd7)
def aurora    : UInt32 := 108   -- nord sage green (#87af87)
def polar     : UInt32 := 236   -- nord dark (#303030)
def snow      : UInt32 := 253   -- nord light (#dadada)
def steel     : UInt32 := 67    -- steel blue (#5f87af) — nord headers/text
def ice       : UInt32 := 152   -- pale teal (#afd7d7) — nord light highlight
def mist      : UInt32 := 189   -- pale blue-lavender (#d7d7ff) — subtle accent
def pink      : UInt32 := 212   -- dracula pink (#ff87d7)
def purple    : UInt32 := 141   -- dracula purple (#af87ff)
def dusk      : UInt32 := 96    -- muted purple (#875f87) — dracula headers
def lilac     : UInt32 := 182   -- soft lavender (#d7afd7) — dracula light accent
def blush     : UInt32 := 224   -- pale pink (#ffd7d7) — dracula light tint
def orange    : UInt32 := 208   -- gruvbox orange (#ff8700)
def olive     : UInt32 := 142   -- gruvbox yellow-green (#afaf00)
def sand      : UInt32 := 223   -- gruvbox light yellow (#ffd7af)
def cream     : UInt32 := 229   -- gruvbox cream (#ffffaf)
def brown     : UInt32 := 130   -- gruvbox dark brown (#af5f00)
def cornsilk  : UInt32 := 230   -- pale warm yellow (#ffffd7) — gruvbox light tint
def coral     : UInt32 := 204   -- monokai pink-red (#ff5f87)
def violet    : UInt32 := 141   -- monokai purple (#af87ff)
def rosegold  : UInt32 := 181   -- dusty rose (#d7afaf) — monokai light highlight
